/*!******************************************************************************
 * Name            : CandidateList.cc
 * Project         : Voting System
 * Description     : Implementation file for CandidateList
...
void CandidateList::Add(Candidate c) { candidate_list.push_back(c); }
...
Candidate& CandidateList::ReturnLoser() {
  ...
  for (unsigned i = 0; i < candidate_list.size(); i++) {
    ...
    } else if (candidate_list.at(i).get_num_ballots() ==
               lowest.get_num_ballots()) {
      if (candidate_list.at(i)
              .get_votes()
              .get_ballot_list()
              .back()
              .get_ballot_no() <
          lowest.get_votes().get_ballot_list().back().get_ballot_no()) {
        lowest = candidate_list.at(i);
      }
    }
  }
...
// Returns a candidate object for a candidate in the list
...
// number of seats in the election. This method handles randomizing winners in
// the case
...
vector<Candidate> CandidateList::ReturnWinners(int no_seats) {
  ...
  for (int j = 0; j < no_seats; j++) {
    ...
    // and finds the one with the most votes. Does so comparing the candidate
    // stored
    // in highest and the current candidate in the list and changes the highest
    // to the
    // current candidate if the current one has more ballots than the previous
    // set
    // highest candidate.
    for (unsigned i = 0; i < candidate_list.size(); i++) {
      if (candidate_list.at(i).get_num_ballots() > highest.get_num_ballots()) {
    ...
    // Removes the candidate with the most votes from the original candidate
    // list
...
int CandidateList::ListSize() { return candidate_list.size(); }
